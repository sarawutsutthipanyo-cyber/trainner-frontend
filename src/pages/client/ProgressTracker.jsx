...
import { getMyProfile, getProgressLogs, addProgressLog, deleteProgressLog } from '../../api';
...
export default function ProgressTracker() {
  ...
  useEffect(() => {
    getMyProfile().then(async (r) => {
      setClient(r.data);
      const l = await getProgressLogs(r.data.id);
      setLogs(l.data);
    }).catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const r = await addProgressLog(client.id, form);
        ...
        {latest && (
          <div className={styles.summaryRow}>
            <div className="stat-card">
              ...
              {weightDiff !== null && <p className={styles.diff} style={{ color: Number(weightDiff) < 0 ? 'var(--success)' : 'var(--danger)' }}>
                {Number(weightDiff) > 0 ? '+' : ''}{weightDiff} kg จากเริ่มต้น
              </p>}
            </div>
            <div className="stat-card"><p className="stat-label">เป้าหมาย</p><p className="stat-value">{client?.targetWeight || '-'} <span className="stat-unit">kg</span></p>{client?.targetWeight && <p className={styles.diff}>เหลืออีก {Math.abs(Number(latest.weight) - Number(client.targetWeight)).toFixed(1)} kg</p>}</div>
            ...
            <div className="stat-card"><p className="stat-label">จำนวนครั้งวัด</p><p className="stat-value">{logs.length} <span className="stat-unit">ครั้ง</span></p></div>
          </div>
        )}
          ...
          {logs.length === 0 ? (
            ...
            <div className={styles.tableWrap}>
              <table className="table">
                <thead>
                  <tr>
                    <th>วันที่</th><th>น้ำหนัก</th><th>เอว</th><th>อก</th><th>สะโพก</th><th>แขน</th><th>ต้นขา</th><th>หมายเหตุ</th><th></th>
                  </tr>
                </thead>
                <tbody>
                  {[...logs].reverse().map((l) => (
