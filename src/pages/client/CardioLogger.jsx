...
import { getMyProfile, getCardioLogs, addCardioLog, deleteCardioLog } from '../../api';
...
export default function CardioLogger() {
  ...
  useEffect(() => {
    getMyProfile().then(async (r) => {
      setClient(r.data);
      const l = await getCardioLogs(r.data.id);
      setLogs(l.data);
    }).catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const r = await addCardioLog(client.id, form);
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteCardioLog(id); setLogs((prev) => prev.filter((l) => l.id !== id)); } catch {}
  };
